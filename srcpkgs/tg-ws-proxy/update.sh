#!/bin/bash
# Auto-updater for tg-ws-proxy (skip if no .deb asset)
set -euo pipefail
...
echo "Fetching latest tg-ws-proxy release..."
...
ASSET_NAME="TgWsProxy_linux_amd64.deb"
...
if [ "$CURRENT" = "$VERSION" ]; then
    echo "tg-ws-proxy: $CURRENT — already up to date"
    exit 0
fi

echo "tg-ws-proxy: $CURRENT → $VERSION"
