...
API_URL="https://api.github.com/repos/anilibria/anilibria-winmaclinux/releases/latest"
LATEST_TAG=$(curl -sL "$API_URL" | python3 -c "import sys,json; print(json.load(sys.stdin)['tag_name'])")
LATEST_VER="${LATEST_TAG#v}"

if [ "${CURRENT}" = "${LATEST_VER}" ]; then
    echo "aniliberty: ${CURRENT} — already up to date"
    exit 0
fi

echo "aniliberty: ${CURRENT} → ${LATEST_VER}"

DEB_URL="https://github.com/anilibria/anilibria-winmaclinux/releases/download/${LATEST_TAG}/aniliberty-amd64.deb"
echo "URL: ${DEB_URL}"
