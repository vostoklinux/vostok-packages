...
echo "Fetching latest curlie release..."

CURL_ARGS=(-fsSL -H "Accept: application/vnd.github+json")
[ -n "${GITHUB_TOKEN:-}" ] && CURL_ARGS+=(-H "Authorization: Bearer ${GITHUB_TOKEN}")

INFO=$(curl "${CURL_ARGS[@]}" \
    "https://api.github.com/repos/rs/curlie/releases/latest") || {
...
for a in d.get('assets', []):
    name = a['name']
    if name.startswith('curlie_') and name.endswith('_linux_amd64.deb'):
        print(name)
        break
" 2>/dev/null)
...
if [ "$CURRENT" = "$LATEST" ]; then
    echo "curlie: $CURRENT — already up to date"
    exit 0
fi

echo "curlie: $CURRENT → $LATEST"
