...
LATEST_VER="${LATEST_TAG#core@}"

if [ "${CURRENT}" = "${LATEST_VER}" ]; then
    echo "insomnia: ${CURRENT} — already up to date"
    exit 0
fi

echo "insomnia: ${CURRENT} → ${LATEST_VER}"

DEB_URL="https://github.com/Kong/insomnia/releases/download/${LATEST_TAG}/Insomnia.Core-${LATEST_VER}.deb"
echo "URL: ${DEB_URL}"
