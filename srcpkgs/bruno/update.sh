...
API_URL="https://api.github.com/repos/usebruno/bruno/releases/latest"
LATEST_TAG=$(curl -sL "$API_URL" | python3 -c "import sys,json; print(json.load(sys.stdin)['tag_name'])")
LATEST_VER="${LATEST_TAG#v}"

if [ "${CURRENT}" = "${LATEST_VER}" ]; then
    echo "bruno: ${CURRENT} — already up to date"
    exit 0
fi

echo "bruno: ${CURRENT} → ${LATEST_VER}"

DEB_URL="https://github.com/usebruno/bruno/releases/download/${LATEST_TAG}/bruno_${LATEST_VER}_amd64_linux.deb"
echo "URL: ${DEB_URL}"
