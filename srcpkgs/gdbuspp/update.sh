...
echo "Fetching latest GDBus++ version from Codeberg..."

INFO=$(curl -fsSL -H "Accept: application/json" \
    "https://codeberg.org/api/v1/repos/OpenVPN/gdbuspp/tags?limit=50") || {
...
if [ "${CURRENT}" = "${LATEST}" ]; then
    echo "gdbuspp: ${CURRENT} — already up to date"
    exit 0
fi

echo "gdbuspp: ${CURRENT} → ${LATEST}"

DOWNLOAD_URL="https://swupdate.openvpn.net/community/releases/gdbuspp-${LATEST}.tar.xz"
