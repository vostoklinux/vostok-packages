...
echo "Fetching latest chiaki-ng release..."
...
# Ищем ассет AppImage (имя chiaki-ng.AppImage_x86_64)
ASSET_NAME=$(echo "$INFO" | python3 -c "
import sys, json
d = json.load(sys.stdin)
for a in d.get('assets', []):
    name = a['name']
    if name.endswith('.AppImage') or name.endswith('.AppImage_x86_64'):
        print(name)
        break
" 2>/dev/null)

if [ -z "$ASSET_NAME" ]; then
    echo "No AppImage asset found in this release. Skipping update."
    exit 0
fi
...
if [ "$CURRENT" = "$LATEST" ]; then
    echo "chiaki-ng: $CURRENT — already up to date"
    exit 0
fi

echo "chiaki-ng: $CURRENT → $LATEST"
...
sed -i "s|^distfiles=.*|distfiles=\"https://github.com/streetpea/chiaki-ng/releases/download/v\${version}/chiaki-ng.AppImage_x86_64\"|" "$TEMPLATE"
